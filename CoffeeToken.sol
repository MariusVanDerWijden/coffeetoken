...
contract CoffeeToken is StandardToken {
    ...
    mapping (address => uint256) timeSinceLastPayout;
    uint256 maxFaucetPayoutPerRound;
    uint32 timeBetweenPayouts;
    uint256 supply;
    ...
    function CoffeeToken() {
        ...
        decimals = 5;
        symbol = "CFEE";
        maxFaucetPayoutPerRound = 10; //10 Token per Payout
        timeBetweenPayouts = 3*24*60*60; //Payout every 3 days
    ...
    function approveAndCall(address _spender, uint256 _value, bytes _extraData) returns (bool success) {
        ...
        if(!_spender.call(bytes4(bytes32(
                sha3("receiveApproval(address,uint256,address,bytes)"))), msg.sender, _value, this, _extraData))
        { throw; }
        return true;
    }
}